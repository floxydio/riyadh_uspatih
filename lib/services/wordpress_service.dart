...
class WordPressService {
  ...
  Future fetchPostsBySearch(String searchText) async {
    var response = WpConfig.blockedCategoryIds.isEmpty
     ? await http.get(Uri.parse("${WpConfig.websiteUrl}/wp-json/wp/v2/posts?search=$searchText"))
     : await http.get(Uri.parse("${WpConfig.websiteUrl}/wp-json/wp/v2/posts?search=$searchText&categories_exclude=" + WpConfig.blockedCategoryIds));
    List? decodedData = jsonDecode(response.body);
    List<Article>? articles;
